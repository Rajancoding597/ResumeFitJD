...
public class LatexResumeService {
    ...
    private List<BulletSegment> parseItemBlocks(String latex) {
        ...
        for (int index = 0; index < tokens.size(); index++) {
            ...
            int rawStart = findItemContentStart(latex, token.end());
    ...
    private int findItemContentStart(String source, int afterItemCommandIndex) {
        int index = afterItemCommandIndex;
        while (index < source.length() && Character.isWhitespace(source.charAt(index))) {
            index++;
        }

        // Handle \\item[Label] syntax by skipping the optional bracketed label.
        if (index < source.length() && source.charAt(index) == '[') {
            int closing = findMatchingBracket(source, index);
            if (closing > index) {
                index = closing + 1;
                while (index < source.length() && Character.isWhitespace(source.charAt(index))) {
                    index++;
    ...
    private int findMatchingBracket(String source, int openingBracketIndex) {
        int depth = 0;
        for (int index = openingBracketIndex; index < source.length(); index++) {
